...
//!
//!     let client_credentials: ClientCredentials = oauth.client_credentials().await?.json().await?;
...
//!     if revoke_token.status() != 200 {
//!         let token_err: TokenError = revoke_token.json().await?;
